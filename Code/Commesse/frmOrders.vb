...
#Region "Menu"
    ...
    Private Sub Modify()
        ...
        rec.Customer = txtCustomerName.Text
        rec.Ended = chkOrdedEnded.Checked
        Dim newOrderName As String = txtOrderRename.Text

        If xDatabase.OrderExists(rec.OrderName) Then
            If newOrderName <> "" Then
                If rec.OrderName <> newOrderName Then
                    If MsgBox("Rinominare commessa " & rec.OrderName & " in " & newOrderName & "? 
Sei sicuro?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2) = MsgBoxResult.Yes Then
                        Dim RowSelected As Integer = dgv.SelectedRows(0).Index
                        xDatabase.ModifyOrderName(rec, newOrderName)
                        LoadData(Filter)
                        dgv.ClearSelection()
                        dgv.Rows(RowSelected).Selected = True
                        If RowSelected > 5 Then
                            dgv.FirstDisplayedScrollingRowIndex = RowSelected
                        Else
                            dgv.FirstDisplayedScrollingRowIndex = 0
                        End If
                        txtOrderName.Text = txtOrderRename.Text
                        txtOrderRename.Text = ""
                    Else
                    End If
                Else
                    MsgBox("Il nuovo nome deve essere diverso dall'originale!", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly)
                End If
            Else
                MsgBox("Nuovo nome commessa vuoto!", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly)
            End If
        Else
            MsgBox("Nome originale commessa non trovato!", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly)
